import { Scene, ArcRotateCamera, Vector3, Vector4, HemisphericLight, SpotLight, MeshBuilder, StandardMaterial, Texture, Color3, Space, ShadowGenerator, PointLight, CreateCylinder, } from "@babylonjs/core";
function createBox(scene, px, py, pz, sx, sy, sz, rotation) {
...
function createFacedBox(scene, px, py, pz) {
    ...
    box.position = new Vector3(px, py, pz);
    scene.registerAfterRender(function () {
        box.rotate(new Vector3(2, 6, 4), 0.02, Space.LOCAL);
    });
    return box;
}
function createTorus(scene, px, py, pz, sx, sy, sz, rotation, rx, ry, rz, rs) {
    const torus = MeshBuilder.CreateTorus("torus", {});
    torus.position = new Vector3(px, py, pz);
    torus.scaling = new Vector3(sx, sy, sz);
    if (rotation) {
        scene.registerAfterRender(function () {
            torus.rotate(new Vector3(rx, ry, rz), rs, Space.LOCAL);
        });
    }
    return torus;
}
function createPolyhedra(scene, t, s, px, py, pz) {
    const polyhedra = MeshBuilder.CreatePolyhedron("shape", { type: t, size: s }, scene);
    polyhedra.position = new Vector3(px, py, pz);
    scene.registerAfterRender(function () {
        polyhedra.rotate(new Vector3(0, 8, 0), 0.02, Space.LOCAL);
    });
    return polyhedra;
}
function createAnyLight(scene, index, px, py, pz, colX, colY, colZ, mesh) {
    switch (index) {
        case 1:
...
function createSphere(scene, px, py, pz) {
...
function createCylinder(scene, px, py, pz, sx, sy, sz) {
    let cylinder = CreateCylinder("cylinder", { height: 1, diameter: 0.7 }, scene);
    cylinder.position = new Vector3(px, py, pz);
    cylinder.scaling = new Vector3(sx, sy, sz);
    scene.registerAfterRender(function () {
        cylinder.rotate(new Vector3(4, 8, 2), 0.02, Space.LOCAL);
    });
    return cylinder;
}
function createCone(scene, px, py, pz, sx, sy, sz) {
    let cone = MeshBuilder.CreateCylinder("cone", { height: 1, diameterBottom: 0.7, diameterTop: 0 }, scene);
    cone.position = new Vector3(px, py, pz);
    cone.scaling = new Vector3(sx, sy, sz);
    scene.registerAfterRender(function () {
        cone.rotate(new Vector3(4, 8, 6), 0.02, Space.LOCAL);
    });
    return cone;
}
function createGround(scene, w, h, px) {
    let ground = MeshBuilder.CreateGround("ground", { width: w, height: h }, scene);
...
export default function createStartScene(engine) {
    ...
    that.box = createBox(that.scene, -8, 2.5, 0, 0.5, 5, 9, false);
    that.box = createBox(that.scene, -3, 2.5, 0, 0.5, 5, 9, false);
    that.box = createBox(that.scene, 3, 2.5, 0, 0.5, 5, 9, false);
    that.box = createBox(that.scene, 7, 2.5, 0, 0.5, 5, 9, false);
    that.box = createBox(that.scene, 12, 2.5, 0, 0.5, 5, 9, false);
